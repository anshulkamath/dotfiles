...
def main():
    ...
    # Write state file for claude-sessions overview (keyed by TMUX_PANE id)
    try:
        tmux_pane = os.environ.get("TMUX_PANE")
        if tmux_pane and state.get("status"):
            import pathlib
            state_dir = pathlib.Path("/tmp/claude-sessions")
            state_dir.mkdir(exist_ok=True)
            (state_dir / tmux_pane.lstrip("%")).write_text(json.dumps({
                "status": state["status"],
                "cwd": cwd,
                "session_id": session_id,
                "pane_id": tmux_pane,
